...
- **Cross-platform через `manage.py`.** Запуск/остановка процесса через `psutil` (Linux / macOS / Windows). Slash-команды просто оборачивают `python manage.py {start,stop,status}`
